...
select
  ...
  'Trained image segmentation models that can separate pet foregrounds from backgrounds on the Oxford-IIIT Pet dataset. The implementation compares UNet and ResNet34-UNet, builds the full PyTorch training and evaluation pipeline, and uses Dice score plus inference outputs to measure segmentation quality.',
...
select
  ...
  'Used a pretrained VQGAN tokenizer with a Masked Bidirectional Transformer to reconstruct masked image regions through iterative token prediction. The work explores mask scheduling, transformer-based visual token modeling, image inpainting inference, and FID-based evaluation of generative quality.',
...
select
  ...
  'Implemented stochastic video generation with a learned prior to predict future dance frames from conditional signals. This project studies Conditional VAE training, the reparameterization trick, KL annealing, teacher forcing schedules, reconstruction loss, and PSNR evaluation over long video sequences.',
...
select
  ...
  'Implemented Deep Q-Network agents and variants for discrete control tasks. The project emphasizes reinforcement learning fundamentals including experience replay, target networks, value estimation, exploration, evaluation rollouts, and recorded videos that make policy behavior easier to inspect.',
...
select
  ...
  'Built a conditional denoising diffusion model that generates iCLEVR images from multi-label object conditions. The implementation combines a conditional UNet, cosine noise scheduling, classifier-free guidance, AdamW training, inference scale sweeps, and evaluator-based accuracy checks.',
...
select
  ...
  'Applied reinforcement learning to traffic signal control using SUMO and a Taipei road network. The project connects simulation engineering with RL policy design by building road topology from real data, constructing SUMO scenarios, and evaluating adaptive signal-control agents in traffic environments.',
